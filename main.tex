...
Using the projected gradient dynamics from Section~\ref{sec:setup}, with \( b_i^{(t)} = 0 \), we obtain:
\begin{equation}
\label{eq:stage0-M1-dynamics}
\begin{aligned}
\langle w_i^{(t)}, \mathbf{M}_{1j} \rangle 
&= \left(1 + \frac{\eta C_1}{d}\right)^t \cdot \frac{\langle w_i^{(0)}, \mathbf{M}_{1j} \rangle + \langle v_i^{(0)}, \mathbf{M}_{1j} \rangle}{2},
\end{aligned}
\end{equation}
where \( C_1 = \Theta(\alpha_1^2) = \Theta(1) \).

For the weaker component \( \mathbf{M}_2 \), the dynamics satisfy:
\begin{equation}
\label{eq:stage0-M2-dynamics}
\begin{aligned}
\langle w_i^{(t)}, \mathbf{M}_{2j} \rangle 
&= \left(1 + \frac{\eta C_2}{d}\right)^t \cdot \frac{\langle w_i^{(0)}, \mathbf{M}_{2j} \rangle + \langle v_i^{(0)}, \mathbf{M}_{2j} \rangle}{2},
\end{aligned}
\end{equation}
where \( C_2 = \Theta(\alpha_2^2) = \Theta(1/d^{2c_0}) \ll C_1 \).

This disparity in growth rates ensures that \( \mathbf{M}_1 \) alignment dominates during Stage~0, while \( \mathbf{M}_2 \) alignment remains close to initialization.
